...
const FIELDS = {
  ...
  staffNumber: 'fldqDXmV76wTQaIby',
  rescheduledDate: 'fldgXZs40WTbkVuI7',
  rescheduledTime: 'fldhhWnJDyljyqMpN',
};
...
function toAirtableFields(friendlyFields) {
  ...
  for (const [key, value] of Object.entries(friendlyFields)) {
    ...
    if (
      fieldId &&
      !fieldId.includes('PLACEHOLDER') &&
      value !== undefined &&
      value !== null &&
      value !== ''
    ) {
      // Staff number is a Single line text field in Airtable.
      // Always send it as plain text.
      if (key === 'staffNumber') {
